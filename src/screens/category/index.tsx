...
export default observer((props: any) => {
    ...
    const { data: result } = useQuery(GQL.categoryQuery, {
        variables: { id: category.id },
    });
    const categoryData = Helper.syncGetter('category', result);
    ...
    return (
        <Query query={GQL.categoryArticlesQuery} variables={{ category_id: category.id }} fetchPolicy="network-only">
            {({ loading, error, data, refetch, fetchMore }) => {
                ...
                if (loading || !articles || !categoryData) return <SpinnerLoading />;
                return (
                    <PageContainer contentViewStyle={{ marginTop: 0 }} error={error}>
                        ...
                        <View style={styles.buttonWrap}>
                            <TouchFeedback activeOpacity={1} onPress={buttonPress}>
                                <Image
                                    source={require('@src/assets/images/ic_send_post.png')}
