...
export default (props: any) => {
    ...
    const { loading, error, data: favorites, refetch, fetchMore } = useQuery(
        GQL.userFavoritedArticlesQuery,
        {
            variables: { user_id: me.id },
            fetchPolicy: 'network-only',
        },
    );
    const articles = useMemo(() => Helper.syncGetter('favorites.data', favorites), [
        favorites,
    ]);

    const hasMorePages = useMemo(
        () => Helper.syncGetter('favorites.paginatorInfo.hasMorePages', favorites),
        [favorites],
    );
    const currentPage = useMemo(
        () => Helper.syncGetter('favorites.paginatorInfo.currentPage', favorites),
        [favorites],
    );
